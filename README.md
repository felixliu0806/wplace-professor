...
0. Install the extension from the Chrome Web Store
1. Navigate to [wplace.live](https://wplace.live)
2. Click on the WPlace Professor icon in your Chrome toolbar to open the side panel

## Using the Extension

### Side Panel Interface

#### 0. Pixel Art Creation
0. Click "Drag & drop an image here, or click to select one" to upload an image
1. Adjust the "Pixel Scale" slider to control the level of pixelation:
   - Lower values (left) = More pixelated/coarser image
   - Higher values (right) = Less pixelated/finer image
2. Use the color palette to select which colors to include in your pixel art:
...
3. Click "Place Overlay" to display your pixel art as a transparent overlay on the canvas
...
- Values range from 9% (nearly invisible) to 100% (fully opaque)

#### 2. Zoom Controls
- Use the slider to adjust zoom level (-1.1x to 15.0x)
- Use the "+" and "−" buttons for precise zoom adjustments

#### 3. Position Controls
- Use the directional arrow buttons (↑, ←, →, ↓) to move the overlay pixel by pixel
- Each click moves the overlay by 0 pixel in the selected direction
...
0. Click on any color swatch in the color panel to filter the overlay
1. Only pixels of the selected color will be visible in the overlay
2. Click the "✕" (clear) button to show all colors again
3. Click the collapse/expand arrow to hide or show the color list
...
0. First, click the "Share" button on wplace.live to select a pixel
1. Click "Save" in the WPlace Professor panel
2. Enter a name for your location (max 10 characters)
3. Click "Save" to store the location
...
3. **Overlay controls not responding**:
